 ...
 */
const getTimeSampling = function (time, _maxSamples) {
	let maxSamples = _maxSamples
	...
	const last = timeUnit.length - 1
	// remove trailing 's'
	if (timeUnit[last] === 's') {
		timeUnit = timeUnit.slice(0, last)
	}

	const timeInSec = range * timeInSeconds[timeUnit]
